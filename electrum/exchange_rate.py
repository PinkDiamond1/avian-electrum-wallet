...
from .i18n import _
from .util import (AvianValue, ThreadJob, make_dir, log_exceptions, OldTaskGroup,
...
class Bittrex(ExchangeBase):
    ...
    async def get_currencies(self):
        dicts = await self.get_json('api.bittrex.com',
                                    '/v3/markets')
        return [d['symbol'][4:] for d in dicts if d['symbol'][:4] == 'AVN-']

    async def get_rates(self, ccy):
        dicts = await self.get_json('api.bittrex.com',
                                    '/v3/markets/AVN-%s/ticker' % ccy)
    ...
    async def request_history(self, ccy):
        dicts = await self.get_json('api.bittrex.com',
                                    'v3/markets/AVN-%s/candles/TRADE/DAY_1/recent' % ccy)
...
class FxThread(ThreadJob, EventListener):
    ...
    def format_amount_and_units(self, btc_balance, *, timestamp: int = None) -> str:
        if timestamp is None:
            ...
            rate = self.timestamp_rate(timestamp)
        if isinstance(btc_balance, AvianValue):
            btc_balance = btc_balance.avn_value.value

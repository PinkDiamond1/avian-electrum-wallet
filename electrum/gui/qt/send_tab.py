...
from electrum.avian import COIN
from electrum.util import (AssetAmountModified, UserFacingException, get_asyncio_loop, bh2u,
                           ...
                           NoDynamicFeeEstimates, InvoiceError, parse_max_spend, AvianValue)
...
class SendTab(QWidget, MessageBoxMixin, Logger):
    ...
    def __init__(self, window: 'ElectrumWindow'):
        ...
        # Let user choose to send AVN or Asset
        ...
        msg = (_("Recipient of the funds.") + "\n\n"
               + _("You may enter a Avian address, a label from your list of contacts "
                   "(a list of completions will be proposed), "
                   "or an alias (email-like address that forwards to a Avian address)") + ". "
    ...
    def spend_max(self):
        ...
        __, x_fee_amount = run_hook('get_tx_extra_fee', self.wallet, tx) or (None, AvianValue())
        ...
        if amount_after_all_fees.assets:
            ...
            amount_after_all_fees = amount_after_all_fees.avn_value.value

        self.amount_e.setAmount(amount_after_all_fees)
        # show tooltip explaining max amount
        mining_fee = tx.get_fee()
        # Mining fees will only be avn
    ...
    def pay_onchain_dialog(
        ...
        output_values = [x.avian_value for x in outputs]
        if any(parse_max_spend(outval) for outval in output_values):
            output_value = '!'
        else:
            output_value = sum(output_values, AvianValue())
    ...
    def get_frozen_balance_str(self) -> Optional[str]:
        frozen_bal = sum(self.wallet.get_frozen_balance(), AvianValue())
        if frozen_bal == AvianValue():
            return None

...
from .avian import address_to_script
from .transaction import PartialTxOutput, AvianValue
...
class Invoice(StoredObject):
    ...
    def generate_amount(x):
        if isinstance(x, AvianValue):
            return x
        return AvianValue.from_json(x)
    ...
    def get_amount_msat(self) -> Union[AvianValue, None]:
    ...
    def get_amount_sat(self) -> Union[AvianValue, None]:
        """
    ...
    def _validate_amount(self, attribute, value: Optional[AvianValue]):
        if value is None:
            return
        if value.assets:
            assert value.avn_value == 0
        ...
        if isinstance(value.avn_value, (Satoshis, int)):
            if not (0 <= value.avn_value <= TOTAL_COIN_SUPPLY_LIMIT_IN_BTC * COIN * 1000):
                raise InvoiceError(f"amount is out-of-bounds: {value!r} msat")
        elif isinstance(value.avn_value, str):
            if value.avn_value != '!':

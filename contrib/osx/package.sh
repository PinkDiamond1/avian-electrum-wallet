...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-Avian.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-avian-macos
	cd /tmp/electrum-avian-macos
...
rm -rf /tmp/electrum-avian-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-avian-macos/image/
cp -r $1 /tmp/electrum-avian-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum_Avian_uncompressed.dmg \
    /tmp/electrum-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum_Avian_uncompressed.dmg electrum-avian-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_Avian_uncompressed.dmg

echo "Done."
sha256sum electrum-avian-$VERSION.dmg

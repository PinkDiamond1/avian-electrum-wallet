...
"$python" -m pip install --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-avian.txt"
...
"$python" -m pip install --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-avian-binaries.txt"
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
...
cp "$PROJECT_ROOT/electrum/gui/icons/electrum-avian.png" "$APPDIR/electrum-avian.png"
